...
from baselines import create_DQN_agent, create_RW_agent
...
from models import CVAE_MiniGrid_Separate2, Encoder_MiniGrid_Separate, Decoder_MiniGrid_Separate
...
interval_beta = 5000
unique_goals = False
local_comments = f"AE2_beta_interval{interval_beta:g}"
...
sample_layout, sample_mask_agent = layout_extractor(obs2tensor(env.reset()))
...
path_tf_events = f"tb_records/{env.spec.id}/{args.size_world}x{args.size_world}/{args.method}/{args.comments}_CVAE_{num_categoricals}x{num_categories}_depth{depth}_width{width}_atoms{atoms}_beta{beta:g}_{local_comments}/{args.seed}"
...
while True:
    ...
    while not done and agent.steps_interact <= args.steps_max:
        ...
        if hrb.get_stored_size() > size_batch_cvae and agent.steps_interact % 4 == 0:
            ...
            batch_processed = process_batch(batch, prioritized=prioritized_cvae, with_targ=True, obs2tensor=obs2tensor, device=DEVICE, aux=False)
            cvae.train()
            (
                loss_overall,
                loss_recon,
